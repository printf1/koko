...
then
  ...
  sh $SHELL_DIR/curl_atr.sh $1 $6 "failed" 'false'
...
do
  ...
  if [ $selectPSCnt -eq 0 ] ; then
    ...
    #python ${PYTHON_PATH}/end_asynchronous_job.py $1 $6 $3 "process execute success"
    sh $SHELL_DIR/curl_atr.sh $1 $6 "success" 'true'
    outlog_func I "${myname} is end."
    exit 0
  else
...
if [ -n "${pid_rs}" ]; then 
  ...
  if [ $? -eq 0 ]; then
    ...
    sh $SHELL_DIR/curl_atr.sh $1 $6 "process_killed" 'false'
  else
    outlog_func E "${pid_rs} is killed error"
    #python ${PYTHON_PATH}/end_asynchronous_job.py $1 $6 $4 "$execRst"
    sh $SHELL_DIR/curl_atr.sh $1 $6 "process_killed_error" 'false'
  fi
