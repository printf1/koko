...
then
  ...
  sh $SHELL_DIR/curl_atr.sh $1 $6 false failed 環境設定ファイルが存在しません
  exit 1
fi
. ${_exec_ksh}
...
then
  ...
  sh $SHELL_DIR/curl_atr.sh $1 $6 false failed 共通関数ファイルが存在しません
  exit 1
fi
...
then
  ...
  sh $SHELL_DIR/curl_atr.sh $1 $6 false フォーマットエラー
...
then
  ...
  sh $SHELL_DIR/curl_atr.sh $1 $6 false failed ファイル存在ない
  exit 2
fi
...
then
  ...
  sh $SHELL_DIR/curl_atr.sh $1 $6 false failed ファイル空き
  exit 2
fi
  ...
  if [ -e ${monitoring_log_file1} ]; then
    ...
    if [ -n "$tmp" -a "${pid}" ]; then    
       ...
       if [ $? -eq 0 ]; then
         ...
         sh $SHELL_DIR/curl_atr.sh $1 $6 false failed 異常終了
...
#search the process of S002_Shell_001.sh by keyword : "S002_Shell_001.sh*{ticketNumber}"
  ...
  if [ $sh001PSCnt -eq 0 ]; then  
    ...
    #python ${PYTHON_PATH}/end_asynchronous_job.py $1 $6 $3 "process execute success"
    sh $SHELL_DIR/curl_atr.sh $1 $6 true success 正常終了
	outlog_func E "正常終了: date '+%Y-%m-%d %H:%M:%S'"
	outlog_func I "${myname} is end."
    exit 0
  else    
