...
trootpwd=${14}                           # 転送モード(binary or ascii)
tmode==${15}                             # 対象serverのrootパスワード
#SDATE          = `date '+%Y%m%d'`

echo "$number getkiban.sh Start！【`date '+%Y-%m-%d %H:%M:%S'`】" >> log_comment
#ステータス定義
...
EOF
y=102  
echo "$number getkiban.sh End！【`date '+%Y-%m-%d %H:%M:%S'`】" >> log_comment     #正常終了の戻り値を設定する
fi                                   