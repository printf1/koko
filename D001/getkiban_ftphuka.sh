...
trootpwd=${11}                           # 転送モード(binary or ascii)
tmode==${12}                             # 対象serverのrootパスワード
#SDATE          = `date '+%Y%m%d'`

echo "$number getkiban_ftphuka.sh Start！【`date '+%Y-%m-%d %H:%M:%S'`】" >> log_comment
#ステータス定義
...
EOF
y=102                                       #正常終了の戻り値を設定する
echo "$number getkiban_ftphuka.sh End！【`date '+%Y-%m-%d %H:%M:%S'`】" >> log_comment
fi
